...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Aurea Lead Generation form submission received:', { 
      fullName, 
      contactNumber, 
      emailAddress,
      preferredDate,
    ...
    return NextResponse.json({
      success: true,
      message: 'Thank you for your interest in Aurea! We have sent you a confirmation email and our team will contact you soon to arrange your showflat visit.',
    ...
    console.error('Aurea Lead Generation form error:', error)
    return NextResponse.json(
...
async function sendNotificationEmail({ 
  ...
  try {
    ...
    console.log('Notification email configuration (Aurea Lead):', {
      hasApiKey: !!apiKey,
      fromEmail,
      toEmail
    })
    ...
    const emailContent = {
      to: toEmail,
      from: fromEmail,
      subject: 'New Aurea Lead Generation - KW Singapore',
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background-color: #B40101; color: white; padding: 20px; text-align: center;">
            ...
            <p style="margin: 5px 0 0 0; font-size: 16px;">Aurea Lead Generation</p>
          </div>
    ...
    try {
      ...
      console.log('✅ Notification email sent successfully via SendGrid (Aurea Lead)')
    } catch (sendError) {
...
async function insertIntoGoogleSheets({ 
  ...
  try {
    ...
    const targetRange = process.env.GOOGLE_SHEETS_AUREA_LEAD_RANGE || 'ShowflatVisitRequests!A:J'

    console.log('Google Sheets configuration (Aurea Lead):', {
    ...
    const dataRow = [
      ...
      'Far East Organization, Perennial Holdings & Sino Land',
      preferredDate || '',
      preferredTiming || '',
      'Aurea Lead Generation'
    ]
    ...
    console.log('✅ Data inserted into Google Sheets successfully (Aurea Lead)')
...
async function sendAutoReplyEmail({ fullName, emailAddress }: {
  ...
  try {
    ...
    console.log('Auto-reply email configuration (Aurea Lead):', {
      hasApiKey: !!apiKey,
      fromEmail,
    ...
    const emailContent = {
      to: emailAddress,
      from: fromEmail,
      subject: 'Aurea Showflat Visit Request Confirmation - KW Singapore',
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <div style="background-color: #B40101; color: white; padding: 20px; text-align: center;">
            <h1 style="margin: 0; font-size: 24px;">KW Singapore</h1>
            <p style="margin: 5px 0 0 0; font-size: 16px;">Aurea Showflat Visit</p>
          </div>
          ...
          <div style="padding: 30px; background-color: #f9f9f9;">
            ...
            <div style="background-color: white; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
              ...
              <p style="color: #666; line-height: 1.6; margin: 10px 0 0 0;">
                Thank you for your interest in Aurea! We have received your showflat visit request and our team will contact you within 24 business hours to arrange your visit.
              </p>
            </div>
    ...
    try {
      await sgMail.send(emailContent)
      console.log('✅ Auto-reply email sent successfully via SendGrid (Aurea Lead)')
    } catch (sendError) {
      console.error('❌ SendGrid send error:', sendError)
      throw sendError
    }
